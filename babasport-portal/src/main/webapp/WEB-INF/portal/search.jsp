...
<script >
	function fqBrand(brandId) {
        window.location.href = "search?keyword=" + $("#keyword").val()+"&brandId="+brandId+"&pa=${pa}"+"&pb=${pb}"+"&solt=${solt1}";
    }
    ...
    function fqPrice(a,b) {
	 //   alert(a+"==="+b)
		if(a==null&&b==null){
		 //   alert("hh");
            window.location.href = "search?keyword=" + $("#keyword").val()+"&solt=${solt1}";
            return;
		}
        window.location.href = "search?keyword=" + $("#keyword").val()+"&brandId=${brandId}"+"&pa="+a+"&pb="+b+"&solt=${solt1}";
    }
...
<div id="J_crumbsBar" class="crumbs-bar">
	...
	<div class="crumbs-nav">
		<div class="crumbs-nav-main clearfix">
			...
			<div class="crumbs-nav-item">
				<strong class="search-key">"${m.key }"</strong>
			</div>
				...
				<span class="crumbs-arrow">已选条件：</span>
					<c:forEach items="${map }" var="m">
						<a title="${m.value }" href="javascript:;" class="crumb-select-item">
							<b>${m.key }：</b><em>${m.value }</em><i onclick="fqPrice()"></i>
						</a>
					</c:forEach>
			</div>
			</c:if>
		...
		<div class="sl-value">
			<div class="sl-v-list">
				<ul class="J_valueList">
					<li>
						<a href="javascript:;" onclick="fqPrice(this.text.split('-')[0],this.text.split('-')[1])"><i></i>0-99</a>
					</li>
					<li>
						<a href="javascript:;" onclick="fqPrice(this.text.split('-')[0],this.text.split('-')[1])"><i></i>100-299</a>
					</li>
					<li>
						<a href="javascript:;" onclick="fqPrice(this.text.split('-')[0],this.text.split('-')[1])"><i></i>300-599</a>
					</li>
					<li>
						<a href="javascript:;" onclick="fqPrice(this.text.split('-')[0],this.text.split('-')[1])"><i></i>600-999</a>
					</li>
					<li>
						<a href="javascript:;" onclick="fqPrice(this.text.split('-')[0],this.text.split('-')[1])"><i></i>1000-1599</a>
					</li>
					<li>
			...
			<div id="J_filter" class="filter">
				<div class="f-line top">
					<div class="f-sort"><a
							...
							href="javascript:;"
							class="">销量<i></i></a><a
							onclick="serachKeyword('${solt}',${superPojo.pageNum},${superPojo.pageSize},)"
							href="javascript:;"
							class="">价格<i></i></a><a
					...
					<div id="J_topPage" class="f-pager">
						<span class="fp-text"><b>${superPojo.pageNum}</b><em>/</em><i>${superPojo.pages}</i></span><a
							class="fp-next"
							onclick="serachKeyword('${solt1}',${superPojo.pageNum-1},${superPojo.pageSize},${superPojo.pages})"
							href="javascript:;">&lt;</a><a
							class="fp-next"
							onclick="serachKeyword('${solt1}',${superPojo.pageNum+1},${superPojo.pageSize},${superPojo.pages})"
							href="javascript:;"
					...
					<div class="f-result-sum">共<span
							id="J_resCount"
							class="num">${superPojo.total}</span>件商品
					</div>
					<span class="clr"></span>
				</div>
				<div class="f-line">
...
<div id="J_goodsList" class="goods-list-v1 gl-type-1 J-goods-list">
	<ul class="gl-warp clearfix" data-tpl="1">
		<c:forEach items="${superPojo.result}" var="product">
		<li data-sku="1711416562" class="gl-item">
			<div class="gl-i-wrap">
				<div class="p-img">
					<a href="javascript:;" onclick="window.open('http://localhost:8084/html/product/${product.id}.html')" style="position: relative;">
